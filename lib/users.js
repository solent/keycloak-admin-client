...
/**
  ...
  @param {number} [options.lifespan] - Number of seconds after which the generated token expires
  @returns {Promise} A promise that resolves.
  @example
  keycloakAdminClient(settings)
    .then((client) => {
      client.users.executeActionsEmail(realmName, userId, { verifyEmail: true, updateProfile: true, updatePassword: true, configureTOTP: true, lifespan: 3600 })
        .then(() => {
          console.log('success')
      })
    })
...
function executeActionsEmail (c) {
  return function executeActionsEmail (realmName, userId, options) {
    return new Promise((resolve, reject) => {
      ...
      if ((options.redirectUri != null) && (options.redirectUri !== '')) {
        if ((options.clientId != null) && (options.clientId !== '')) {
          queryParameters.redirectUri = options.redirectUri;
          queryParameters.clientId = options.clientId;
        } else {
      ...
      if (options.lifespan != null) {
        queryParameters.lifespan = options.lifespan;
      }

      const req = {
        url: `${c.baseUrl}/admin/realms/${realmName}/users/${userId}/execute-actions-email`,
        auth: {
          bearer: privates.get(c).accessToken
        },
        json: true,
